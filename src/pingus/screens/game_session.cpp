  ...
  single_step(false)
{
  ...
  playfield = gui_manager->create<Playfield>(get_server(), this,
                                             Rect(Vector2i(std::max((Display::get_width()  - world_width)/2,  0),
  ...
  time_display = gui_manager->create<TimeDisplay>(this);
...
GameSession::update(const input::Event& event)
{
  ...
  switch (event.type)
  {
    ...
    default:
      // unhandled event
      log_info("GameSession::process_events (): unhandled event: {}", static_cast<int>(event.type));
      break;
  }
}
