...
public:
  ...
  constexpr Vector3f (const Vector3f& old) noexcept = default;
  constexpr Vector3f (Vector2f old) noexcept : x(old.x), y(old.y), z(0) {}
  constexpr Vector3f (Vector2i old) noexcept : x(float(old.x)), y(float(old.y)), z(0) {}
  constexpr Vector3f& operator= (const Vector3f& old) noexcept = default;
  ...
  constexpr Vector3f operator+ (Vector3f add) const noexcept
  {
    return Vector3f(x + add.x, y + add.y, z + add.z);
  }
  constexpr Vector3f operator- (Vector3f sub) const noexcept
  {
  ...
  constexpr Vector3f& operator+= (Vector3f add) noexcept
  {
    x += add.x;
    y += add.y;
    z += add.z;
  ...
  constexpr Vector3f& operator-= (Vector3f sub) noexcept
  {
    x -= sub.x;
    y -= sub.y;
    z -= sub.z;
  ...
  static float distance2d(Vector3f a, Vector3f b)
  {
    float x = b.x - a.x;
    float y = b.y - a.y;
  ...
  static constexpr Vector3f interpolate(Vector3f a, Vector3f b, float perc) noexcept
  {
    Vector3f c = b - a;
    return a + (c * perc);
  }
