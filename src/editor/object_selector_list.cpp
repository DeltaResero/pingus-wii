...
ObjectSelectorList::on_pointer_move (int x, int y)
{
  ...
  if (mode == SCROLLING)
  {
    ...
    offset = std::clamp(offset, std::min(static_cast<float>(rect.get_height()) - (static_cast<float>(height) * 48.0f), 0.0f), 0.0f);
  }
}
...
ObjectSelectorList::scroll(float y)
{
  if (set)
  {
    ...
    int height = (static_cast<int>(set->get_objects().size()) / width) +
      ((static_cast<int>(set->get_objects().size()) % width > 0) ? 1 : 0);
    offset = std::clamp(offset, std::min(static_cast<float>(rect.get_height()) - (static_cast<float>(height) * 48.0f),
                                   0.0f), 0.0f);
  }
}
