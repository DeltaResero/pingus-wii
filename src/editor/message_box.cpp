  ...
  on_ok()
{
  ...
  m_ok_button->on_click = std::bind(&MessageBox::on_ok_button, this);
  m_cancel_button->on_click = std::bind(&MessageBox::on_cancel_button, this);
}
...
MessageBox::draw_background(DrawingContext& gc)
{
  ...
  // main text
  gc.print_center(pingus::fonts::verdana11, Vector2i(rect.get_width()/2, 42),
                  m_text);
}
