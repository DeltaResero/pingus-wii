...
void log_message(logmich::LogLevel level, std::string_view prefix, std::string_view fmt, Args&&... args) {
    if (level <= logmich::current_log_level.load(std::memory_order_relaxed)) {
        std::ostream& out = (level == logmich::kError || level == logmich::kWarning) ? std::cerr : std::cout;
        try {
            out << prefix << std::vformat(fmt, std::make_format_args(args...)) << std::endl;
        } catch (const std::format_error& e) {
            out << prefix << "!!! FORMAT ERROR: " << e.what() << " | Original format string: " << fmt << " !!!" << std::endl;
        }
    }
}

// Variadic macros that forward to the templated function.
#define log_error(fmt, ...) log_message(logmich::kError, "[ERROR] ", fmt, ##__VA_ARGS__)
#define log_warn(fmt, ...)  log_message(logmich::kWarning, "[WARN]  ", fmt, ##__VA_ARGS__)
#define log_info(fmt, ...)  log_message(logmich::kInfo,  "[INFO]  ", fmt, ##__VA_ARGS__)
#define log_debug(fmt, ...) log_message(logmich::kDebug, "[DEBUG] ", fmt, ##__VA_ARGS__)
#define log_tmp(fmt, ...)   log_message(logmich::kTmp,   "[TMP]   ", fmt, ##__VA_ARGS__)
